...
for f in files:
  ...
  # print(f"{label} - {category} - {participant}")
...
merged_df.resample(rule="200L").apply(sampling)

days = [g for n, g in merged_df.groupby(pd.Grouper(freq='D'))]

data_resampled = pd.concat([df.resample(rule="200L").apply(sampling).dropna() for df in days])
...
# Export dataset
# --------------------------------------------------------------
data_resampled.to_pickle("../../data/interim/01data_resampled.pkl")